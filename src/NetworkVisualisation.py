...
from src.PlotUtils import plot_to_grid
...
class NetworkVisualisation(object):
    ...
    def p2w_changed(self, val):
        if not self.ignore_update:
            self.network.layers[self.layer][self.p2c, 0] = val
    ...
    def p2c_changed(self, val):
        if not self.ignore_update:
            self.p2c = int(val)
            self.ignore_update = True
            self.p2w_slid.set_val(self.network.layers[self.layer][self.p2c, self.neuron2])
