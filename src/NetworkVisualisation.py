...
class NetworkVisualisation(object):
    ...
    def __init__(self, X: np.ndarray, y: np.ndarray, network: Network, quality: int, min_range: float = None, max_range: float = None, saves_path: str = None, seed: int = 1):
        np.random.seed(seed)
        ...
        self.data_points = X
        self.data_labels = y
        self.data_space, self.dim_data = setup_data_space(self.min_range, self.max_range, quality)
        ...
        self.neuron2 = 0
        self.right_all = True
        self.layer = self.network.n_layers-1
        self.p2c = 0
    ...
    def plot_network(self, fig):
        ...
        for i in range(self.network.n_classes):
            self.layer1_plot.ax.scatter(grouped_points[i][:, 0], grouped_points[i][:, 1], s=3, c=np.random.rand(1, 3), alpha=0.5)

        self.layer1_3d_plot = Plot3D(fig, (4, 4), (1, 0), (1, 3), self.precision, results[2])

        self.layer2_plot = Plot(fig, (4, 4), (2, 0), (1, 3), results[(self.layer+1)*2][:, :, 0], self.min_range, self.max_range)
        for i in range(self.network.n_classes):
            self.layer2_plot.ax.scatter(grouped_points[i][:, 0], grouped_points[i][:, 1], s=3, c=np.random.rand(1, 3), alpha=0.5)

        self.layer2_3d_plot = Plot3D(fig, (4, 4), (3, 0), (1, 3), self.precision, results[(self.layer+1)*2])
    ...
    def plot_controls(self, fig):
        ...
        p1x_ax = plot_to_grid(fig, (2, 17), (0, 12), (1, 1))
        ...
        p1y_ax = plot_to_grid(fig, (2, 17), (0, 13), (1, 1))
        ...
        p1b_ax = plot_to_grid(fig, (24, 17), (0, 14), (7, 1))
        ...
        p1_ax = plot_to_grid(fig, (24, 17), (0, 15), (7, 1))
        ...
        p1_opt_ax = plot_to_grid(fig, (24, 17), (8, 14), (3, 3))
        self.p1_opt_buttons = CheckButtons(p1_opt_ax, ["ReLU?", "Cut?", "Enabled?", "All?"], [self.is_relu, self.cut, True, self.left_all])
        self.p1_opt_buttons.on_clicked(self.p1_options_update)

        # Plot 2 Controls
        layer_2 = self.network.to_np(self.network.layers[self.layer])
        bias_2 = self.network.to_np(self.network.biases[self.layer])
        ...
        p2_weight_val_ax = plot_to_grid(fig, (2, 17), (1, 12), (1, 1))
        ...
        p2_connection_dim_ax = plot_to_grid(fig, (2, 17), (1, 13), (1, 1))
        ...
        p2b_ax = plot_to_grid(fig, (24, 17), (13, 14), (7, 1))
        ...
        p2_ax = plot_to_grid(fig, (24, 17), (13, 15), (7, 1))
        ...
        if self.network.n_layers-1 != 1:
            layer_ax = plot_to_grid(fig, (24, 17), (13, 16), (7, 1))
            self.layer_slid = Slider(layer_ax, 'l', valmin=1, valmax=self.network.n_layers-1, valinit=self.layer, valstep=1)
            self.layer_slid.on_changed(self.layer_changed)

        p2_opt_ax = plot_to_grid(fig, (24, 17), (21, 14), (4, 3))
        self.p2_opt_buttons = CheckButtons(p2_opt_ax, ["Pre-add?", "Transform?", "Enabled?", "All?"], [self.is_pre_add, self.is_sig, True, self.right_all])
    ...
    def p1_options_update(self, label):
        if not self.ignore_update:
            if label == "ReLU?":
                ...
                self.left_all = not self.left_all

            self.update_visuals()

    def p2w_changed(self, val):
        if not self.ignore_update:
            self.network.layers[self.layer][self.p2c, self.neuron2] = val
    ...
    def p2b_changed(self, val):
        if not self.ignore_update:
            self.network.biases[self.layer][0, 0] = val
            self.update_just_plot2()
    ...
    def p2_options_update(self, label):
        if not self.ignore_update:
            if label == "Transform?":
                ...
                if is_enabled and self.neuron2 not in self.selected_architecture[self.layer+1]:
                    self.selected_architecture[self.layer+1].add(self.neuron2)
                elif not is_enabled and self.neuron2 in self.selected_architecture[self.layer+1]:
                    layer2_out = sorted(list(self.selected_architecture[self.layer+1])).index(self.neuron2)
                    self.layer2_3d_plot.remove_plot(layer2_out)
                    self.selected_architecture[self.layer+1].remove(self.neuron2)
            elif label == "All?":
                self.right_all = not self.right_all
            self.update_just_plot2()
    ...
    def update_3d_plot1(self, out1, out2):
        if not self.ignore_update:
            if self.neuron1 in self.selected_architecture[1]:
                if not self.is_relu:
                    self.layer1_3d_plot.update_all(out1, self.neuron1, self.left_all)
                else:
                    self.layer1_3d_plot.update_all(out2, self.neuron1, self.left_all)

    def update_plot2(self, out2, out3, out4):
        if not self.ignore_update:
            if self.neuron2 in self.selected_architecture[self.layer+1]:
                self.layer2_plot.set_visible(True)
                neuron2_index = sorted(list(self.selected_architecture[self.layer+1])).index(self.neuron2)
                if self.is_pre_add:
                    layer2_data = scale_out2(out2, self.network.to_np(self.network.layers[self.layer]), self.selected_architecture[self.layer], neuron2_index, self.precision)
    ...
    def update_3d_plot2(self, out2, out3, out4):
        if not self.ignore_update:
            if self.is_pre_add:
                layer2_data = scale_out2(out2, self.network.to_np(self.network.layers[self.layer]), self.selected_architecture[self.layer], self.neuron2, self.precision)
            elif not self.is_sig:
                layer2_data = out3
            else:
                layer2_data = out4
            self.layer2_3d_plot.update_all(layer2_data, self.neuron2, self.right_all)

    def update_visuals(self):
        if not self.ignore_update:
            results = [self.network.to_np(result) for result in
                       self.network.forward(self.network.from_np(self.data_space), arch_selection=self.selected_architecture, precision=self.precision, is_mlp=self.cut)]
            self.update_plot1_visuals(results[1], results[2])
            self.update_plot2_visuals(results[(self.layer+1)*2-2], results[(self.layer+1)*2-1], results[(self.layer+1)*2])
            plt.draw()
    ...
    def update_plot2_widgets(self):
        self.p2b_slid.set_val(self.network.to_np(self.network.biases[self.layer])[0, self.neuron2])
        self.p2w_slid.set_val(self.network.to_np(self.network.layers[self.layer])[self.p2c, self.neuron2])
        self.p2w_slid.vline.set_xdata(self.original_layers[self.layer][self.p2c, self.neuron2])
        self.p2c_slid.set_val(self.p2c)
        self.p2c_slid.valmax = self.network.layers[self.layer].shape[0]-1
        self.p2c_slid.ax.set_xlim(0, self.p2c_slid.valmax)
        self.p2_slid.set_val(self.neuron2)
        self.p2_slid.valmax = self.network.layers[self.layer].shape[1]-1
        self.p2_slid.ax.set_xlim(0, self.p2_slid.valmax)

        self.p2b_slid.vline.set_xdata(self.original_biases[self.layer][0, self.neuron2])
        self.p2w_slid.vline.set_xdata(self.original_layers[self.layer][self.p2c, self.neuron2])

        if (self.neuron2 in self.selected_architecture[self.layer+1] and not self.p2_opt_buttons.get_status()[2]) or \
                (self.neuron2 not in self.selected_architecture[self.layer+1] and self.p2_opt_buttons.get_status()[2]):
